...
export const updateUserProfile = async (req, res) => {
    try {
        const address = req.user.address; // ✅ updated line
        const { name, email, avatar, bio } = req.body;
        const user = await User.findOneAndUpdate({ address }, { name, email, avatar, bio }, { new: true, upsert: false });
        if (!user) {
...
export const getUserProfileByAddress = async (req, res) => {
    try {
